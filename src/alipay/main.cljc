(ns alipay.main
  "Kotodama WASM entrypoint for the Alipay clean-room actor (L4) — Clojure port.
  ...
  facts (`alipay.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "alipay")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"alipay.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "alipay-compat" :tier tier :entities entities} 200])
...
(defn start! [] :alipay-compat/ready)
